...
export default function AddCarScreen(props) {
  ...
  const handleRegisterButton = async () => {
    if (name != '' && Color != '' && Model != '' && Make != '' && RegNo != '') { //check to leave no field emty
      const FormData = {
        name: name,
        color: Color,
        model: Model,
      ...
      const resp = await axios.get(Url + '/cars?regno=' + RegNo); //Every car must have a unique registration numbers
