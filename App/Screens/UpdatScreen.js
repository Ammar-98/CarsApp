...
export default function UpdateScreen(props) {
  ...
  useEffect(() => {
    setName(props.route.params.item.name);  //the fields are prefilled so we can know the previously saved values
  ...
  const handleUpdateButton = async () => {
    if (name != '' && Color != '' && Model != '' && Make != '' && RegNo != '') { //No field shall be left empty
      const FormData = {
        name: name,
        color: Color,
        model: Model,
      ...
      const res = await axios.put(
        Url + '/cars/' + props.route.params.item.id,  //the item id is taken from props and updated
        FormData,
      );
      console.log('res', res);
  ...
  return (
    <View style={styles.AllArea}>
      <View style={styles.header}>
        <Text style={styles.headerText}>Update car data</Text>
      </View>
